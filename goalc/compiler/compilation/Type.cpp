...
Val* Compiler::generate_inspector_for_structure_type(const goos::Object& form,
  ...
  constraint.desired_register = emitter::get_register_info(m_instr_set).get_gpr_arg_reg(0);  // to the first argument
...
Val* Compiler::generate_inspector_for_bitfield_type(const goos::Object& form,
  ...
  constraint.ireg = input_arg->ireg();  // constrain this register
  if (bitfield_128) {
    constraint.desired_register = emitter::get_register_info(m_instr_set).get_xmm_arg_reg(0);  // to the first argument
  } else {
    constraint.desired_register = emitter::get_register_info(m_instr_set).get_gpr_arg_reg(0);  // to the first argument
  }
