...
# 1: File Setup
To create a custom level, copy the layout of `custom_assets/jak2/levels/test-zone`. See `test-zone.jsonc` for information on how to name things. The `.gd` file also contains the level name.

# 2: Modify the engine
Modify the Jak 2 level metadata under `goal_src/jak2/engine/level/` to add level info for each custom level. There is level info for `test-zone` at the bottom that can be used as an example.

# 3: Modify the build system
Modify `goal_src/jak2/game.gp` and add a custom level target:
```lisp
...
Blender will create a `.glb` file, which must have the name specified in the `.jsonc` file and should be located in `custom_assets/jak2/levels/your_level`
